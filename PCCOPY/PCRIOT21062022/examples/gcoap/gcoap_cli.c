    ...
    switch (method_flag) {
        ...
        case COAP_PUT:
            ...
            if (pdu->payload_len <= 10) {
                ...
                printf("%s\n", payload);
                req_count = (uint16_t)strtoull(payload, NULL, 10);
                printf("%u\n", pdu->payload_len);
...
int gcoap_cli_cmd(int argc, char **argv)
{
    ...
    if (((argc == apos + 2) && (code_pos == 0)) ||    /* ping */
        ...
        size_t paylen = (argc == apos + 4) ? strlen(argv[apos+3]) : 0;//11111111111111111
        if (paylen) {
            coap_opt_add_format(&pdu, COAP_FORMAT_TEXT);
        }
        ...
        if (paylen) {
            ...
            if (pdu.payload_len >= paylen) {
                memcpy(pdu.payload, argv[apos+3], paylen);//11111111111111111
                len += paylen;
            }
            else {
                puts("gcoap_cli: msg buffer too small");
